...
export const Flux2ProImageForm: React.FC<Flux2ProImageFormProps> = ({ onSubmit, isLoading, apiKey = '' }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-zinc-900/80 p-6 border border-zinc-800 relative">
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-orange-600 via-pink-500 to-orange-600"></div>
      ...
      <div className="bg-zinc-800/50 border border-zinc-700 p-3 text-xs text-zinc-400 font-mono">
        ...
        <div className="text-[10px] text-zinc-500">
          Model: flux-2/pro-image-to-image • Upload 1-8 reference images
        </div>
      </div>
              ...
              {uploadingIndex === index ? (
                ...
                <>
                  <input
                    ...
                    className="hidden"
                    id={`flux2pro-img-upload-${index}`}
                  />
                  <label htmlFor={`flux2pro-img-upload-${index}`} className="cursor-pointer flex flex-col items-center">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Describe how to transform the images (e.g., 'Change the man into the outfit shown in picture two')..."
          required
        />
      ...
      <div className="text-[10px] text-zinc-600 font-mono text-center border-t border-zinc-800 pt-4">
        API: POST /api/v1/jobs/createTask • Model: flux-2/pro-image-to-image
      </div>
    </form>
  );
};
