...
interface SettingsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (kieKey: string, googleKey: string) => void;
  currentKey: string;
  currentGoogleKey: string;
}

export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSave, currentKey, currentGoogleKey }) => {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        setTempGoogleKey(currentGoogleKey);
    }
  }, [isOpen, currentKey, currentGoogleKey]);

  if (!isOpen) return null;

  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    onSave(tempKey, tempGoogleKey);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-zinc-900 border border-zinc-700 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-6">
           ...
           <div>
              <label className="block text-xs text-blue-500 mb-2 font-mono uppercase">Google Gemini API Key (Direct)</label>
              <div className="relative">
                <input 
                    type="password" 
                    value={tempGoogleKey}
                    onChange={(e) => setTempGoogleKey(e.target.value)}
                    className="w-full bg-black border border-zinc-700 p-3 pr-10 text-white focus:border-blue-500 focus:outline-none font-mono text-sm"
                    placeholder="AIza..."
                />
                <div className="absolute right-3 top-3 text-zinc-600">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 19l-1 1-1 1-2-2 2-2 2-2 2-2-4-4m9 9a2 2 0 01-2-2m-2-2l-4-4"></path></svg>
