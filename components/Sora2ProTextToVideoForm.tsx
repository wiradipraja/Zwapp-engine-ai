...
import { Sora2ProTextToVideoInput } from '../types';
import { Button } from './ui/Button';
import { useTheme } from '../contexts/ThemeContext';

interface Sora2ProTextToVideoFormProps {
  onSubmit: (input: Sora2ProTextToVideoInput) => void;
  isLoading: boolean;
  apiKey?: string;
}

export const Sora2ProTextToVideoForm: React.FC<Sora2ProTextToVideoFormProps> = ({ onSubmit, isLoading }) => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';

  const [formData, setFormData] = useState<Sora2ProTextToVideoInput>({
    prompt: '',
  ...
  const handleChange = (field: keyof Sora2ProTextToVideoInput, value: any) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className={`p-3 border ${isDark ? 'border-zinc-800 bg-zinc-900/50' : 'border-zinc-200 bg-zinc-50'}`}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className={`text-xs font-mono tracking-wider ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
            SORA 2 PRO TEXT→VIDEO
          </span>
        </div>
        <p className={`text-[10px] font-mono ${isDark ? 'text-zinc-600' : 'text-zinc-500'}`}>
          Model: sora-2-pro-text-to-video • Higher quality video generation
        </p>
      </div>
      ...
      <div className={`text-center text-[10px] font-mono ${isDark ? 'text-zinc-700' : 'text-zinc-400'}`}>
        API: POST /api/v1/jobs/createTask • Model: sora-2-pro-text-to-video
      </div>
    </form>
  );
};

export default Sora2ProTextToVideoForm;
